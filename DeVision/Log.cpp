...
const char* LOG_FILE = "log\\log.txt";
...
{
	const std::wstring path = L"log\\";
	const std::wstring date = getDate();
	//获取时间
	std::wstringstream wss;
	SYSTEMTIME sysTime;
	::GetLocalTime(&sysTime);
	wss << std::setw(2) << std::setfill(L'0');
	wss << sysTime.wHour << L"_" << std::setw(2) << std::setfill(L'0')
		<< sysTime.wMinute << L"_" << std::setw(2) << std::setfill(L'0')
		<< sysTime.wSecond;
	const std::wstring time = wss.str();
	const std::wstring temp = L"-";
	const std::wstring name = L".txt";
	...
	file_name += time;
	file_name += name;
	const wchar_t* wlog_file = file_name.c_str();
	_bstr_t wfile(wlog_file);
	const char* LOG_FILE = wfile;
